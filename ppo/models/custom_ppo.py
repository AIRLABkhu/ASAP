        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                for _ in range(num_mc):
                    ...
                    # print(f"policy_distance : {policy_distance.shape}")
                    # print(f"value_distance : {value_distance.shape}")
                v_s = self.policy.predict_values(observations).type(th.float32)
                v_s_bar = self.policy.predict_values(s_bar).type(th.float32)
                ...
                l2c2_loss = th.mean(lambda_pi * d_pi_per_sample + lambda_v * value_distance)
                ...
                loss = policy_loss + self.ent_coef * entropy_loss + self.vf_coef * value_loss + l2c2_loss
                ...
                loss.backward(retain_graph=(actions.shape[-1] == 1))
                # Clip grad norm
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                # 거리 계산 (여기서는 L2 거리 사용)
                loss_t = 0.5 * th.nn.functional.mse_loss(pi_s, pi_s_next)
                loss_s = 0.5 * th.nn.functional.mse_loss(pi_s, pi_s_bar)
                caps_loss = weight_t * loss_t + weight_s * loss_s
                ...
                loss = policy_loss + self.ent_coef * entropy_loss + self.vf_coef * value_loss + caps_loss
...
class LipsPPO(PPO):
    def __init__(
        ...
        self.lips_lam = lips_lam
        self.lips_eps = lips_eps
        self.lips_k_init = lips_k_init
        self.lips_f_size = lips_f_size
        self.lips_k_size = lips_k_size

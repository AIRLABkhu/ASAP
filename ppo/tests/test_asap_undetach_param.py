...
from modules.controller import train_asap_undetach_param
...
alg_cnts = dict({
    "asap" : train_asap_undetach_param
})
...
def mk_param_list_lam(lam_list, lam_t_list, lam_p_list):
    param_list = []
    pth_list=[]
    for lam_t in lam_t_list:
        for lam in lam_list:
            param_list.append(dict(
                ...
                lam_s = lam,
                lam_t = lam_t))
            pth_list.append(f"asap_undetach_ppo_lamS{lam}_lamT{lam_t}_")
...
test_params, pth_list = mk_param_list_lam(args.lams, args.lamt, args.lamp)
max_num = args.num
...
subpath = f"asap_undetached_{args.envs[0]}_lams{args.lams[0]}_lamt{args.lamt[0]}/"

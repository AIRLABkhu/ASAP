...
from modules.controller import train_capst_asap_param
...
def mk_param_list_lam(lam_list, lam_t_list):
    param_list = []
    pth_list=[]
    for lam_t in lam_t_list:
        for lam in lam_list:
            param_list.append(dict(
                ...
                lam_s = lam,
                lam_t = lam_t))
            pth_list.append(f"capst_asap_ppo_lamS{lam}_lamT{lam_t}_")
...
subpath = f"capstasap_{args.envs[0]}_lamt{args.lamt[0]}/"
